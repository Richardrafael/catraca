...
function mask_nome(e){
    value = value.replace(/\d/g, ''); // Remove números
    value = value.replace(/[^\w\sÀ-ú]/g, ''); // Remove caracteres especiais
    value = value.toLowerCase().replace(/(?:^|\s)\S/g, function (a) {
      return a.toUpperCase(); // Capitaliza cada palavra
    });
    return value;
